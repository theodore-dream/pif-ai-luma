...
def level2_handle_level(level, entropy):
...
def handle_option_a(entropy):
    ...
    # Decrease entropy by 2, not going below 0
    entropy = max(0, entropy - 2)
    # Return a result (e.g., a string containing game text)
    return f"Option A chosen. entropy decreased by 2. Current entropy level: {entropy}"

def handle_option_b(entropy):
    # Implement game logic for Option B
    # Increase entropy by 2, not going above 10
    entropy = min(10, entropy + 2)
    # Return a result (e.g., a string containing game text)
    return f"Option B chosen. entropy increased by 2. Current entropy level: {entropy}"
...
def handle_game():
    ...
    if level == 1:
        gametext = poetry_game_intro()
        
    elif level == 2:
        gametext = level2_handle_level(level, entropy)
